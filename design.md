...
│ (agent configuration + all test cases)               │
└───────────────────────┬──────────────────────────────┘
                        │
...
│ • Opens ~/.local/share/jiopc/agent/test_run_<timestamp>.log                 │
│ • Dispatches Parts A, B, and C in configurable order                        │
│ • Writes JSON Lines records and a final summary record                      │
│ • Exits with code 0 if all required tests pass, non-zero otherwise          │
└───────────────┬───────────────────────┬───────────────────────┬─────────────┘
                │                       │                       │
      ...
      │                 │     │                 │     │                 │
      │ Playwright      │     │ Xvfb + psutil   │     │ Dual index      │
      │ Headless        │     │ + subprocess    │     │ (Desktop +      │
      │ Chromium        │     │ monitoring      │     │ Start Menu)     │
      └────────┬────────┘     └────────┬────────┘     └────────┬────────┘
               │                       │                       │
               └───────────────┬───────┴───────────────┬───────┘
                               │
        ...
        ┌─────────────────────────────────────────────────────┐
        │ test_run_<timestamp>.log (JSON Lines)               │
        │                                                     │
        │ • One JSON object per test                          │
        │ • Final summary record at end of file               │
        └───────────────────────┬─────────────────────────────┘
                                │
                     Optional: --analyse
                                │
          ...
          ┌────────────────────────────────────────────┐
          │ analyse.py + analyse_log.txt               │
          │                                            │
          │ • Model-agnostic LLM                       │
          │ • Reads JSON log                           │
          │ • Generates structured report              │
          │ • Final recommendation: PROMOTE / HOLD     │
          └────────────────────────────────────────────┘
```
